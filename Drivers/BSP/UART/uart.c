...
void half_duplex_uart_init(uint32_t baudrate)
{
    ...
    HAL_UART_Receive_IT(&handle_for_half_duplex_uart,rx_buffer, BUFFER_SIZE_FOR_USART1); 
 ...
 */
 ...
 void uart_init(uint32_t baudrate)
 {
     ...
     HAL_UART_Receive_IT(&handle_for_uart,rx_buffer, BUFFER_SIZE_FOR_USART1); 
...
void USART_UX_IRQHandler(void)
{
    ...
    HAL_UART_IRQHandler(&handle_for_uart);   /* 调用HAL库中断处理公用函数 */
