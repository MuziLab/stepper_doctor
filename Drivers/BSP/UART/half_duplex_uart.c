...
static UART_HandleTypeDef handle_for_usart = {0};
...
void half_duplex_uart_init(uint32_t baudrate)
{
    ...
    handle_for_usart.Instance = HALF_DUPLEX_USART_UX;                                       /* USART_UX */
    handle_for_usart.Init.BaudRate = baudrate;                                  /* 波特率 */
    handle_for_usart.Init.WordLength = UART_WORDLENGTH_8B;                      /* 字长为8位数据格式 */
    handle_for_usart.Init.StopBits = UART_STOPBITS_1;                           /* 一个停止位 */
    handle_for_usart.Init.Parity = UART_PARITY_NONE;                            /* 无奇偶校验位 */
    handle_for_usart.Init.HwFlowCtl = UART_HWCONTROL_NONE;                      /* 无硬件流控 */
    handle_for_usart.Init.Mode = UART_MODE_TX_RX;                               /* 收发模式 */
    HAL_HalfDuplex_Init(&handle_for_usart);                                           //这个函数很好玩，我还从没见过这么玩的
    ...
    HAL_UART_Receive_IT(&handle_for_usart,rx_buffer, BUFFER_SIZE_FOR_USART1); 
...
{
    return HAL_UART_Transmit(&handle_for_usart,pData,Size,Timeout);
}

HAL_StatusTypeDef half_duplex_uart_receive(uint8_t *pData, uint16_t Size, uint32_t Timeout)
{
    return HAL_UART_Receive(&handle_for_usart,pData,Size,Timeout);
}
