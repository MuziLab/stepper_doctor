 ...
 */
 ...
 #define PALSE_GPIO_CLK_ENABLE_3()                 do{  __HAL_RCC_GPIOD_CLK_ENABLE(); }while(0)   /* 配置gpio口的宏*/
 #define PALSE_TIMER_3                          TIM4
 #define PALSE_TIMER_IRQn_3                     TIM4_IRQn
 #define PALSE_TIMER_IRQHandler_3               TIM4_IRQHandler
 #define PALSE_TIMER_CHY_3                      TIM_CHANNEL_1                           /* 通道Y */
 #define PALSE_TIMER_CHY_CCRX_3                 TIM4->CCR1                             /* 通道Y的输出比较寄存器,不同通道有对应寄存器 */
 #define PALSE_TIMER_CHY_CLK_ENABLE_3()         do{ __HAL_RCC_TIM4_CLK_ENABLE(); }while(0)  /* TIM8 时钟使能 */
 #define PALSE_TIMER_CHY_CLK_DISABLE_3()         do{ __HAL_RCC_TIM4_CLK_DISABLE(); }while(0)
