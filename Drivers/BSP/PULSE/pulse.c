 ...
 */
 ...
 void palse_init(uint16_t arr, uint16_t psc)
 {
     ...
     palse_timer_chy_handle.Init.AutoReloadPreload = TIM_AUTORELOAD_PRELOAD_ENABLE; /*使能TIMx_ARR进行缓冲 */
     palse_timer_chy_handle.Init.RepetitionCounter = 0;                 /* 重复计数器初始值 */
     HAL_TIM_PWM_Init(&palse_timer_chy_handle);                         /* 初始化PWM */
 
     gpio_init_struct.Pin = PALSE_OUT_GPIO_PIN;                         /* 通道y的CPIO口 */
     gpio_init_struct.Mode = GPIO_MODE_AF_PP;                           /* 复用推挽输出 */
     gpio_init_struct.Pull = GPIO_PULLUP;                               /* 上拉? */
     gpio_init_struct.Speed = GPIO_SPEED_FREQ_HIGH;                     /* 高速 */
     HAL_GPIO_Init(PALSE_OUT_GPIO_PORT, &gpio_init_struct);
     ...
     HAL_TIM_PWM_ConfigChannel(&palse_timer_chy_handle, &timx_oc_npwm_chy, PALSE_TIMER_CHY); /* 配置TIMx通道y */
 
     HAL_NVIC_SetPriority(PALSE_TIMER_IRQn, 1, 3);                   /* 设置中断优先级，抢占优先级1，子优先级3 */
     HAL_NVIC_EnableIRQ(PALSE_TIMER_IRQn);                           /* 开启ITMx中断 */
 
     __HAL_TIM_ENABLE_IT(&palse_timer_chy_handle, TIM_IT_UPDATE);       /* 允许更新中断 */
